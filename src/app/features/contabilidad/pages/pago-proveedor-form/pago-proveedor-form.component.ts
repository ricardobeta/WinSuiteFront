...
interface ProveedorConSaldo extends ProveedorCxpOpcion {
    ...
    <section class="pago-form-page">
      ...
      <section class="surface-card form-card">
        <div class="grid">
          ...
          <mat-form-field appearance="outline"><mat-label>Fecha del pago</mat-label><input matInput [matDatepicker]="pagoPicker" [ngModel]="fechaDate()" (ngModelChange)="actualizarFecha($event)" (input)="limpiarFechaSiVacia($event)" /><mat-datepicker-toggle matIconSuffix [for]="pagoPicker"></mat-datepicker-toggle><mat-datepicker #pagoPicker></mat-datepicker></mat-form-field>
          <mat-form-field appearance="outline"><mat-label>Método de pago</mat-label><mat-select [ngModel]="metodoPago()" (ngModelChange)="metodoPago.set($event)"><mat-option value="TRANSFERENCIA">Transferencia</mat-option><mat-option value="EFECTIVO">Efectivo</mat-option><mat-option value="CHEQUE">Cheque</mat-option><mat-option value="TARJETA">Tarjeta</mat-option><mat-option value="OTRO">Otro</mat-option></mat-select></mat-form-field>
          <mat-form-field appearance="outline"><mat-label>Referencia (cheque / transferencia)</mat-label><input matInput [ngModel]="referencia()" (ngModelChange)="actualizarReferencia($event)" maxlength="60" /></mat-form-field>
        </div>
        <div class="contrapartida"><label class="field-label">Cuenta de origen (HABER — caja/banco)</label><app-cuenta-contable-autocomplete [cuentas]="cuentas()" [cuentaId]="cuentaOrigenId() || null" [soloActivas]="true" [soloMovimiento]="true" [mostrarNumero]="false" label="Cuenta caja/banco" (cuentaSeleccionada)="cuentaOrigenId.set($event?.id ?? '')" /></div>
        ...
        <mat-form-field appearance="outline" class="full"><mat-label>Glosa</mat-label><input matInput [ngModel]="glosa()" (ngModelChange)="editarGlosa($event)" maxlength="180" placeholder="Factura A, B; Referencia 123" /><mat-hint>Se completa con los documentos aplicados y la referencia.</mat-hint></mat-form-field>
...
export class PagoProveedorFormComponent implements OnInit {
  ...
  protected readonly proveedoresConSaldo = computed<ProveedorConSaldo[]>(() => {
    ...
    for (const documento of this.pendientes()) {
      ...
      if (!mapa.has(clave)) mapa.set(clave, { clave, proveedorId: documento.proveedorId ?? null, nombre: documento.proveedorNombre, identificacion: documento.proveedorIdentificacion });
    }
    return Array.from(mapa.values()).sort((a, b) => a.nombre.localeCompare(b.nombre));
  });
  protected readonly documentosProveedor = computed(() => this.proveedorClave() ? this.pendientes().filter((documento) => this.claveProveedor(documento) === this.proveedorClave()) : []);
  ...
  protected actualizarFecha(fecha: Date | null): void { if (fecha) this.fecha.set(dateAIso(fecha)); }
  ...
  protected async registrar(): Promise<void> {
    if (!this.puedeGuardar()) { this.error.set('Selecciona proveedor, cuenta de origen y al menos un abono.'); return; }
    const fecha = fechaPagoLocal(this.fecha());
    if (fecha === null) { this.error.set('Selecciona una fecha de pago válida.'); return; }
    ...
    try {
      ...
      if (await this.integracionContable.contabilidadActiva()) {
        const propuesta = await this.integracionContable.construirLineasPagoProveedor({ cuentaPorPagarId: config.cuentaPorPagarDefaultId, cuentaOrigenId: this.cuentaOrigenId(), proveedorId: proveedor?.proveedorId ?? null, glosa, aplicaciones: aplicaciones.map((aplicacion) => ({ documentoNumero: aplicacion.documentoNumero, monto: aplicacion.monto })) }, { lenient: true });
        const data: RevisarAsientoData = { titulo: 'Revisar asiento del pago', subtitulo: `${proveedor?.nombre ?? ''} · ${glosa}`, lineas: propuesta, cuentas: this.cuentas() };
        const resultado = await firstValueFrom(this.dialog.open<RevisarAsientoDialogComponent, RevisarAsientoData, AsientoContableLinea[] | undefined>(RevisarAsientoDialogComponent, { maxWidth: '96vw', data }).afterClosed());
        if (!resultado) return;
        lineas = resultado;
      }
      await this.service.registrarPago({ proveedorId: proveedor?.proveedorId ?? null, proveedorNombre: proveedor?.nombre ?? '', fecha, cuentaOrigenId: this.cuentaOrigenId(), metodoPago: this.metodoPago(), referencia: this.referencia(), glosa, aplicaciones }, lineas);
      this.snackBar.openFromComponent(SuccessSnackbarComponent, { data: { message: 'Pago registrado.', icon: 'check_circle' }, duration: 2600, horizontalPosition: 'end', verticalPosition: 'top' });
      await this.router.navigate(['/workspace/contabilidad/cuentas-por-pagar/pagos']);
    } catch (error: unknown) { this.error.set(error instanceof Error ? error.message : 'No se pudo registrar el pago.'); }
