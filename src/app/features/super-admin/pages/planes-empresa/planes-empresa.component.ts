...
import { PlanEmpresa } from '../../../../core/models/platform.models';
import { PlatformApiService } from '../../../../core/services/platform-api.service';
import { ConfirmDialogComponent } from '../../../../shared/components/confirm-dialog/confirm-dialog.component';
import { SuccessSnackbarComponent } from '../../../../shared/components/success-snackbar/success-snackbar.component';
import { bytesAMegabytes, formatearBytes, formatearCantidad, megabytesABytes } from '../../utils/formato';

/** Catalogo de planes que se asignan a una empresa, con sus capacidades y limites. */
@Component({
      ...
      <section class="surface-card bloque">
        <div class="cabecera">
          <div>
            ...
            <h2>Planes por empresa</h2>
            <p class="sub">Define que puede hacer cada plan y cuanto puede consumir.</p>
          </div>
        ...
        <div class="tabla">
          <table mat-table [dataSource]="planes()">
            ...
            <ng-container matColumnDef="espacio">
              ...
              <td mat-cell *matCellDef="let row">{{ textoBytes(row.limites?.storageBytes) }}</td>
            </ng-container>

            <ng-container matColumnDef="tokens">
              <th mat-header-cell *matHeaderCellDef>Tokens IA / mes</th>
              <td mat-cell *matCellDef="let row">{{ textoCantidad(row.limites?.aiTokensMes) }}</td>
            </ng-container>

            <ng-container matColumnDef="sitios">
              <th mat-header-cell *matHeaderCellDef>Sitios</th>
              <td mat-cell *matCellDef="let row">
                {{ textoCantidad(row.limites?.sitiosEcommerce) }} ecommerce ·
                {{ textoCantidad(row.limites?.sitiosLanding) }} landing
              </td>
            </ng-container>
        ...
        <form class="surface-card bloque" [formGroup]="form" (ngSubmit)="guardar()">
          ...
          <div class="grid">
            ...
            <mat-form-field appearance="outline">
              ...
              <mat-hint>Se genera del nombre si lo dejas vacio. No se puede cambiar despues.</mat-hint>
            </mat-form-field>
            ...
            <mat-form-field appearance="outline">
              ...
              <input matInput type="number" formControlName="precioAnual" />
            </mat-form-field>
          ...
          <h4>Limites</h4>
          <p class="ayuda">Escribe <strong>-1</strong> en cualquier campo para dejarlo sin limite.</p>
  ...
  styles: [`
    ...
    h4 { margin: .4rem 0 0; font-size: 1rem; }
    ...
    .toggle { display: grid; align-content: center; }
    .modulos { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: .35rem; }
...
export class PlanesEmpresaComponent implements OnInit {
  ...
  protected readonly columnas = ['nombre', 'precio', 'espacio', 'tokens', 'sitios', 'estado', 'acciones'];
  ...
  private cargar(): void {
    this.api.listarPlanesEmpresa().subscribe({
      next: (planes) => this.planes.set(planes),
      error: () => this.error.set('No se pudo cargar el catalogo de planes.'),
    });
  }

  protected textoBytes(valor: number | null | undefined): string {
    return formatearBytes(valor);
  }

  protected textoCantidad(valor: number | null | undefined): string {
  ...
  protected editar(plan: PlanEmpresa): void {
    this.form.patchValue({
      ...
      limites: {
        ...
        facturasSriMes: plan.limites?.facturasSriMes ?? null,
  ...
  protected guardar(): void {
    ...
    const plan: PlanEmpresa = {
      ...
      precioMensual: valores.precioMensual,
      precioAnual: valores.precioAnual,
      moneda: 'USD',
      activo: valores.activo,
      orden: valores.orden,
      visiblePublico: valores.visiblePublico,
      ...
      limites: {
        ...
        facturasSriMes: valores.limites.facturasSriMes,
    ...
    this.api.guardarPlanEmpresa(plan).subscribe({
      next: () => {
  ...
  protected eliminar(plan: PlanEmpresa): void {
    ...
    dialogRef.afterClosed().subscribe((confirmado) => {
      ...
      this.api.eliminarPlanEmpresa(plan.id).subscribe({
        next: () => {
          this.cargar();
